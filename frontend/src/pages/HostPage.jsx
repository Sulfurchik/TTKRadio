...
function HostPage() {
  ...
  useEffect(() => {
    ...
    const syncPreview = async () => {
      ...
      video.muted = true
      if (broadcastStatus?.is_broadcasting && !broadcastStatus?.is_paused) {
        video.play().catch(() => {})
      } else {
        video.pause()
      }
  ...
  const handleStartRenameMedia = (item) => {
    setEditingMediaId(item.id)
    setEditingMediaName(item.original_name)
  }
  ...
  const handleSaveRenameMedia = async (mediaId) => {
    const trimmedName = editingMediaName.trim()
    if (!trimmedName) {
      setNotice({ type: 'error', text: t('host.renameFileError') })
      return
    }

    try {
      await hostService.renameMedia(mediaId, trimmedName)
      await Promise.all([loadMedia(), loadPlaylists(), refreshStatus()])
  ...
  return (
    <div className="container page-shell">
      ...
      <div className="surface-card surface-card--monitor">
        ...
        <div className={`monitor-stage ${isCurrentVideo && isLive ? 'monitor-stage--with-preview' : ''}`}>
          <div className="monitor-stage__main">
            {monitorEnabled && (
                ...
                <div className="monitor-summary__grid">
                  <div>
                    ...
                    <div className="monitor-summary__meta">
                      ...
                      {currentTrack.duration > 0 ? ` / ${formatPlaybackTime(currentTrack.duration)}` : ''}
                    </div>
                  </div>
                </div>
              ) : null}
            ...
            <div className="glass-panel live-mic-panel">
              <div>
                ...
                {(isLiveMicActive || isLiveMicConnecting) && (
                  <div className="monitor-summary__meta" style={{ marginTop: '0.45rem' }}>
                    {t('host.onAir')} · {formatPlaybackTime(playbackSeconds)}
                  </div>
                )}
              </div>
              <div className="surface-card__actions">
          ...
          {isCurrentVideo && isLive && (
            <div className="monitor-preview-shell">
              <div className="monitor-preview-shell__title">{t('host.videoAir')}</div>
              <div className="monitor-preview-shell__time">{formatPlaybackTime(playbackSeconds)}</div>
              <div className="monitor-preview">
                <video ref={monitorVideoRef} className="monitor-preview__video" playsInline muted preload="auto" />
              </div>
            </div>
          )}
        </div>
                  ...
                  <div className="media-card__title-row">
                    {editingMediaId === item.id ? (
                      <input
                        ...
                        placeholder={t('host.renameFilePlaceholder')}
                        autoFocus
                        onKeyDown={(event) => {
                          if (event.key === 'Enter') {
                            event.preventDefault()
                            handleSaveRenameMedia(item.id)
                          }
                          if (event.key === 'Escape') {
                            handleCancelRenameMedia()
                          }
                    ...
                    {editingMediaId === item.id ? (
                      <div className="media-card__actions-inline">
                        <button
                          type="button"
                          className="btn btn-primary btn-sm"
                          onClick={() => handleSaveRenameMedia(item.id)}
                          title={t('host.renameFileSave')}
                          aria-label={t('host.renameFileSave')}
                        >
                          ✓
                        </button>
                        <button
                          type="button"
                          className="btn btn-outline btn-sm"
                          onClick={handleCancelRenameMedia}
                          title={t('host.renameFileCancel')}
                          aria-label={t('host.renameFileCancel')}
                        >
                          ✕
                        </button>
                      </div>
                      ...
                      <button
                        type="button"
                        className="btn btn-outline btn-sm media-card__icon-btn"
                        onClick={() => handleStartRenameMedia(item)}
                        title={t('host.renameFile')}
                        aria-label={t('host.renameFile')}
                      >
                        <svg width="14" height="14" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16.862 5.487l1.651-1.651a2.333 2.333 0 113.3 3.3l-9.626 9.627a4 4 0 01-1.69 1.011l-3.444 1.148 1.148-3.444a4 4 0 011.011-1.69l7.65-7.651zm0 0L19.5 8.125" />
                        </svg>
                      </button>
                    )}
                  </div>
